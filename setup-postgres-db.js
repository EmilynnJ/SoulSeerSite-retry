...
async function setupDatabase() {
  ...
  try {
    ...
    `);
    console.log('Readings table created or exists');
    ...
    `);
    console.log('Livestreams table created or exists');
    ...
    `);
    console.log('Client balances table created or exists');
    ...
    `);
    console.log('Reader balances table created or exists');
    ...
    `);
    console.log('Sessions table created or exists');
    ...
    `);
    console.log('Gifts table created or exists');
    ...
    `);
    console.log('Forum categories table created or exists');
    ...
    `);
    console.log('Forum threads table created or exists');
    ...
    `);
    console.log('Forum posts table created or exists');
    
    // Create test user accounts
    console.log('Creating test user accounts...');
    
    // Hash passwords for our test users
    const adminPasswordHash = await hashPassword('JayJas1423!');
    const readerPasswordHash = await hashPassword('JayJas1423!');
    const clientPasswordHash = await hashPassword('Jade2014!');
    
    // Admin user
    const adminResult = await client.query(`
      INSERT INTO users (
        username, email, password, full_name, role, profile_image, 
        bio, is_verified, is_online
      )
      VALUES (
        'emilynnj14', 'emilynnj14@gmail.com', $1, 'Admin User', 
        'admin', '/images/users/admin.jpg', 'System administrator', 
        TRUE, TRUE
      )
    ...
    // Reader user with specific pricing
    const readerResult = await client.query(`
      INSERT INTO users (
        username, email, password, full_name, role, profile_image, 
        bio, is_verified, is_online, is_available, specialties, 
        years_of_experience, pricing_video, pricing_voice, pricing_chat,
        minimum_session_length
      )
      VALUES (
        'emilynn992', 'emilynn992@gmail.com', $1, 'Psychic Reader', 
        'reader', '/images/users/reader.jpg', 
        'Tarot specialist with 10 years of experience in spiritual guidance.',
        TRUE, TRUE, TRUE, ARRAY['Tarot', 'Mediumship', 'Love Readings'],
        10, 200, 150, 100, 5
      )
      ON CONFLICT (email) DO UPDATE 
      SET username = EXCLUDED.username, 
    ...
    const clientResult = await client.query(`
      ...
      VALUES (
        'emily81292', 'emily81292@gmail.com', $1, 'Client User', 
        'client', TRUE
      )
      ON CONFLICT (email) DO UPDATE 
      SET username = EXCLUDED.username, 
